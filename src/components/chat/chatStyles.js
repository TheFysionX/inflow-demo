...
export const styles = {
    ...
    msgSurface: (isUser, border) => ({
        width: isUser ? "fit-content" : "100%",
        maxWidth: isUser ? "min(78%, 760px)" : "min(100%, 860px)",
        padding: isUser ? "12px 16px" : 0,
        borderRadius: isUser ? 18 : 0,
        background: isUser ? "rgba(255,255,255,0.065)" : "transparent",
        border: "none",
        boxShadow: isUser ? "0 10px 24px rgba(0,0,0,0.16)" : "none",
    ...
    detailToggleArrow: {
        ...
        color: "transparent",
        backgroundImage:
            'url("data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 12 12%27 fill=%27none%27 stroke=%27rgba(255,255,255,0.82)%27 stroke-width=%271.6%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27%3E%3Cpath d=%27M4 2.5 7.5 6 4 9.5%27/%3E%3C/svg%3E")',
    ...
    thinkingStepRow: () => ({
        ...
        gap: 12,
        alignItems: "start",
        animation: "thinkingStepEnter 260ms cubic-bezier(0.2, 0.8, 0.2, 1) both",
    }),
    thinkingStepMarkerCol: () => ({
        position: "relative",
        minHeight: 48,
        display: "flex",
        justifyContent: "center",
    }),
    thinkingStepMarker: (isActive, accent, border) => ({
        position: "relative",
        width: isActive ? 13 : 11,
        height: isActive ? 13 : 11,
        marginTop: 4,
        borderRadius: 2,
        border: `1px solid ${isActive ? withAlpha(accent, 0.68) : withAlpha(border, 0.76)}`,
        background: isActive
            ? withAlpha(accent, 0.22)
            : "rgba(255,255,255,0.12)",
        boxShadow: isActive
            ? `0 0 0 5px ${withAlpha(accent, 0.08)}`
            : "none",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        ...
        transition: "transform 220ms ease, background 220ms ease, border-color 220ms ease, box-shadow 220ms ease, width 220ms ease, height 220ms ease",
    }),
    thinkingStepPulse: (accent) => ({
        width: 5,
        height: 5,
        borderRadius: 1,
        background: withAlpha(accent, 0.96),
        animation: "thinkingDiamondPulse 1.6s ease-in-out infinite",
    }),
    thinkingStepConnector: (accent, border) => ({
        position: "absolute",
        top: 22,
        bottom: -10,
        left: "50%",
        width: 1,
        transform: "translateX(-50%)",
        background: `linear-gradient(180deg, ${withAlpha(accent, 0.28)} 0%, ${withAlpha(border, 0.44)} 100%)`,
        animation: "thinkingConnectorReveal 220ms ease both",
    }),
    thinkingStepContent: () => ({
        display: "grid",
        gap: 4,
    ...
    thinkingStepTitle: (text, scale, isActive) => ({
        color: withAlpha(text, isActive ? 0.96 : 0.86),
        fontSize: round1(15.4 * scale),
        lineHeight: 1.4,
        fontWeight: isActive ? 540 : 480,
        letterSpacing: "-0.012em",
        animation: "thinkingWordSwap 220ms ease both",
        transition: "color 220ms ease, opacity 220ms ease",
    }),
    thinkingStepMeta: (muted, scale, isActive) => ({
        color: withAlpha(muted, 0.88),
        fontSize: round1(13.1 * scale),
        lineHeight: 1.35,
        fontWeight: 450,
        display: "inline-flex",
        alignItems: "center",
        ...
        animation: "thinkingWordSwap 240ms ease both",
